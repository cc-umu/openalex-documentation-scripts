...
def copy_sql(csv_dir: str = DEFAULT_CSV_DIR) -> str:
    lines = ["-- Generated by flatten-openalex-jsonl.py --print-copy-sql.", ""]
    for table_name, columns in TABLES.items():
        column_list = ", ".join(column.name for column in columns)
        lines.append(f"\\copy openalex.{table_name} ({column_list}) FROM PROGRAM 'gunzip -c {csv_dir}/{table_name}.csv.gz' WITH (FORMAT csv, HEADER true)")
    lines.append("")
    return "\n".join(lines)
...
def write_sql_files(sql_dir: Path, csv_dir: str) -> None:
    (sql_dir / "openalex-pg-schema.sql").write_text(schema_sql(), encoding="utf-8")
    (sql_dir / "copy-openalex-csv.sql").write_text(copy_sql(csv_dir), encoding="utf-8")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.print_copy_sql:
        print(copy_sql(args.output_dir))
        return 0
    if args.print_index_sql:
        print(index_sql())
        return 0
    if args.write_sql:
        write_sql_files(Path(args.sql_dir), args.output_dir)
        return 0
